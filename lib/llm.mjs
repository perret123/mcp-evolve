 ...
 *
 * Two modes:
 *   - llm()           — plain text generation (grading, prompt gen, etc.)
 ...
 * For non-tool calls (grading, prompt gen, escalation, etc.).
 */
 ...
 *
 * Returns { response, toolCalls, errors } — same shape as parseStreamOutput.
 */
...
async function localToolLoop(prompt, opts) {
  ...
  try {
    ...
    for (let i = 0; i < maxIterations; i++) {
      ...
      const controller = new AbortController();
      const iterTimeout = opts.iterationTimeout || (_globalConfig && _globalConfig.localIterationTimeout) || 180_000;
      const iterTimer = setTimeout(() => controller.abort(), Math.min(iterTimeout, deadline - Date.now()));
...
function formatToolResult(response, toolCalls, errors) {
  return { __localResult: true, response, toolCalls, errors };
}
