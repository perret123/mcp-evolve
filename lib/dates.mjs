...
export function resolveQuestionDateContext(question, runDateContext) {
  ...
  for (const match of question.matchAll(/\b(today|tomorrow|yesterday)\b/gi)) {
  ...
  for (const match of question.matchAll(/\b(this|next)\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/gi)) {
    const phrase = match[0];
...
export function formatDateContextForPrompt(dateContext) {
  ...
  if (dateContext.resolvedPhrases?.length > 0) {
    lines.push('Resolved relative dates in this question:');
