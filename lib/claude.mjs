...
export function parseStreamOutput(output) {
  ...
  for (const line of output.split('\n')) {
    ...
    try {
      ...
      if (event.type === 'result') {
        ...
        if (event.is_error) errors.push({ tool: 'cli', error: response });
      }
  ...
  // Detect MCP-level errors in the final response text
  if (errors.length === 0 && toolCalls.length > 0 && !response.startsWith('{')) {
    ...
    for (const pattern of errorPatterns) {
      if (responseLower.includes(pattern)) {
        errors.push({
          ...
          error: `[detected in response] ${response.slice(0, 500)}`,
        });
        break;
      }
    }
  ...
  return { response, toolCalls, errors };
}
