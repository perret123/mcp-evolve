...
import { savePromptSet, loadPromptSet } from './eval.mjs';
...
async function proposeFeature(groupName, group, context, config) {
  ...
  const prompt = promptTemplate
    ...
    .replace('{{PASSING_PROMPTS}}', context.passingPrompts || context.passingQuestions || '(none)');
...
async function voteOnProposals(groupName, group, proposals, config) {
  ...
  const proposalText = Object.entries(proposals)
    .map(([key, p]) => `**Group ${key}: "${p.name}"**\n${p.description}\nWhy: ${p.why}\nExample test: ${p.testQuestion || p.testPrompt}`)
    .join('\n\n');
 ...
 *
 * @param {object} context - { passingPrompts, fullContext }
 * @param {object} config
...
export async function runCompetition(context, config) {
  ...
  const proposalContext = {
    knowledge,
    passingPrompts: context.passingPrompts || context.passingQuestions || '',
  };
  ...
  // 5. Generate test prompts and promote to golden set
  const testPrompts = [{
    prompt: winningProposal.testPrompt || winningProposal.testQuestion,
  ...
  // Generate 2 more test prompts from the winning feature
  try {
    const morePromptsOutput = await llm(
      `Generate 2 more test prompts for this new MCP server feature:\n\nFeature: ${winningProposal.name}\nDescription: ${winningProposal.description}\n\nFor each prompt, include a probe (a simple read prompt to check state before/after), an invariant (what should hold), and a probeType ("action" or "read").\n\nReply with JSON: {"prompts": [{"persona": "persona-id", "prompt": "...", "probe": "...", "invariant": "...", "probeType": "action|read"}]}\n\nUse personas from: ${config.personas.map(p => p.id).join(', ')}`,
      { model: config.competitionModel || 'sonnet', timeout: 30_000 },
    );
    const match = morePromptsOutput.match(/\{[\s\S]*\}/);
    if (match) {
      const parsed = JSON.parse(match[0]);
      if (parsed.prompts) {
        for (const q of parsed.prompts) testPrompts.push(q);
      }
    }
  } catch { /* extra prompts optional */ }

  // Spec 2: add competition test prompts to the prompt-set as golden candidates.
  // Task 10 will wire the promoter agent for full promotion logic; for now we
  // add them as golden lifecycle entries directly.
  let promoted = 0;
  const ps = loadPromptSet(config);
  if (ps) {
    for (const q of testPrompts) {
      const pText = typeof q === 'string' ? q : (q.prompt || q.question);
      const personaId = typeof q === 'object' ? q.persona : config.personas[0]?.id;
      if (!pText || !personaId) continue;
      const exists = ps.prompts.some(e => e.persona === personaId && e.prompt === pText);
      if (!exists) {
        ps.prompts.push({
          persona: personaId,
          prompt: pText,
          promptObj: typeof q === 'object' ? q : { prompt: q },
          lifecycle: 'golden',
          evaluation: 'fixer',
          consecutivePasses: 0,
          promotedAt: new Date().toISOString(),
          promotionSource: 'competition',
          competitionFeature: winningProposal.name,
        });
        promoted++;
      }
    }
    if (promoted > 0) savePromptSet(ps, config);
  }

  log(`  Promoted ${promoted} test prompts to golden set`);
  ...
  const feature = {
    ...
    testPrompts: testPrompts.map(q => typeof q === 'string' ? q : (q.prompt || q.question)),
  };
