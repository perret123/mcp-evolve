...
export function validateReviewerOutput(parsed) {
  ...
  if (!parsed || typeof parsed !== 'object') {
    return ['validateReviewerOutput: parsed input is not an object'];
  }

  for (const a of parsed.audits || []) {
    if (!a || typeof a !== 'object') {
      errors.push(`audit entry is not an object: ${JSON.stringify(a)}`);
      continue;
    }

    if (!a.branch || typeof a.branch !== 'string') errors.push(`audit missing branch: ${JSON.stringify(a)}`);
    if (!VALID_FIX_TYPES.has(a.fixType)) errors.push(`audit.fixType invalid for ${a.branch}: ${a.fixType}`);
    if (!VALID_DECISIONS.has(a.decision)) errors.push(`audit.decision invalid for ${a.branch}: ${a.decision}`);
  ...
  for (const pr of parsed.promptReviews || []) {
    if (!pr || typeof pr !== 'object') {
      errors.push(`prompt review entry is not an object: ${JSON.stringify(pr)}`);
      continue;
    }
    ...
    if (!VALID_INVARIANT_STATUS.has(pr.invariantStatus)) errors.push(`prompt review invariantStatus invalid for ${pr.promptId}: ${pr.invariantStatus}`);
    if (!VALID_PROMPT_DECISIONS.has(pr.decision)) errors.push(`prompt review decision invalid for ${pr.promptId}: ${pr.decision}`);
  }
