...
import {
  ...
  compareToBaseline, printRegressionReport, checkDiversity, computeDistributionDrift, getPreviousQuestions,
...
export async function generateQuestions(persona, config, fullContext, runDateContext) {
  ...
  const prompt = [
    ...
    '',
    'Reply with ONLY a JSON object: {"questions": [{"question": "text", "probe": "probe question", "invariant": "rule", "probeType": "action|read"}]}',
  ].join('\n');
...
export async function answerQuestion(question, persona, config, runDateContext, overrides = {}) {
  ...
  if (isLocalModel(effectiveModel)) {
    ...
    const output = await llmWithTools(promptText, {
    ...
    // llmWithTools returns { __localResult, answer, toolCalls, errors }
    result = output.__localResult
      ? { answer: output.answer, toolCalls: output.toolCalls, errors: output.errors }
      : parseStreamOutput(output);
  } else {
    // Claude CLI: existing path
    const output = await claude(promptText, {
      systemPrompt: readPrompt(config.promptsDir, 'answerer.md'),
      mcpConfig: config.mcpConfig,
  ...
  if (config.mcpToolPrefix && !matchedExpectedTool && (toolNames.length > 0 || answerMentionsUnavailableTools) && !usingLocalModel) {
    result.errors.push({
...
export async function gradeAnswer(question, result, persona, config, runDateContext, probeContext = {}) {
  ...
  const probeSection = probeContext.beforeProbe && probeContext.afterProbe ? [
    ...
    `\n### Before (state before main question)`,
    `Probe question: ${probeContext.probe}`,
    `Probe answer: ${probeContext.beforeProbe.answer.slice(0, 1500)}`,
    `\n### After (state after main question)`,
    `Probe question: ${probeContext.probe}`,
    `Probe answer: ${probeContext.afterProbe.answer.slice(0, 1500)}`,
  ].join('\n') : '';
...
async function escalate(allPassingQuestions, prefetchData, config, runDateContext) {
  ...
  // Drift guard: check if escalated questions diverge from baseline distribution
  const baseline = loadBaseline(null, config);
  const goldenSet = loadGoldenSet(config);
  const referenceQs = [
    ...(baseline?.questions || []).map(q => q.question),
    ...(goldenSet?.questions || []).map(q => q.question),
  ];
  if (referenceQs.length > 0) {
    ...
    if (drift.highDrift) {
      ...
      if (config.driftAction === 'reject') {
        log('  Escalation questions rejected due to high drift');
...
export async function run(config, args = {}) {
  ...
  async function answerAndGrade(questionObj, persona) {
    const qText = typeof questionObj === 'string' ? questionObj : questionObj.question;
    const probe = typeof questionObj === 'object' ? questionObj.probe : null;
    const invariant = typeof questionObj === 'object' ? questionObj.invariant : null;
    const probeType = typeof questionObj === 'object' ? questionObj.probeType : null;

    // 1. Before-probe (if probe exists)
    let beforeProbe = null;
    if (probe) {
      log(`    [probe:before] "${probe.slice(0, 60)}..."`);
      beforeProbe = await answerQuestion(probe, persona, config, runDateContext, {
        model: config.probeModel || 'haiku',
      });
    }

    // 2. Main question (answerer sees only text)
    const result = await answerQuestion(qText, persona, config, runDateContext);

    // 3. After-probe (if probe exists)
    let afterProbe = null;
    if (probe) {
      log(`    [probe:after] "${probe.slice(0, 60)}..."`);
      afterProbe = await answerQuestion(probe, persona, config, runDateContext, {
        model: config.probeModel || 'haiku',
      });
    }
    ...
    if (!args.skipGrading && result.errors.length === 0) {
      const grading = await gradeAnswer(qText, result, persona, config, runDateContext, {
        probe, beforeProbe, afterProbe, invariant, probeType,
      });
    ...
    if (probe) {
      result.probeData = {
        ...
        beforeAnswer: beforeProbe?.answer?.slice(0, 500),
        afterAnswer: afterProbe?.answer?.slice(0, 500),
        invariantResult: result.grading?.invariantResult || 'skipped',
      };
    }

    const writeToolCalled = result.toolCalls.some(tc => matchesWriteTools(tc.tool, config));
    const isAction = isActionRequestFn(qText, config.actionVerbs);
    ...
    const score = scoreQuestion({ question: qText, ...result }, config);
    result.score = score;
    return { question: qText, questionObj, persona, ...result };
  }
  ...
  if (useFixedSets && !isRegression && !dryRun) {
    ...
    const eligible = questionSet.questions.filter(q => selectedIds.has(q.persona) && !q.obsolete);
    ...
    const questionsToRun = [...trainQs, ...goldenQs];

    log(`Running ${questionsToRun.length} questions in PARALLEL${eligible.length > questionsToRun.length ? ` (sampled from ${eligible.length})` : ''}`);

    // Answer + grade ALL questions in parallel
    const results = await Promise.all(
      questionsToRun.map(q => {
        const persona = config.personas.find(p => p.id === q.persona);
        return answerAndGrade(q.questionObj || q.question, persona);
      })
    );
  ...
  // === LEGACY MODE (no question-set.json) ===
    ...
    async function runPersona(persona) {
      ...
      const goldenQuestions = goldenForPersona.map(q => q.questionObj || q.question);

      let questions;
      if (isRegression) {
        questions = regressionQuestions.filter(q => q.persona === persona.id).map(q => q.questionObj || q.question);
      ...
      if (goldenQuestions.length > 0) {
        const questionTexts = new Set(questions.map(q => typeof q === 'string' ? q : q.question));
        const newGolden = goldenQuestions.filter(gq => !questionTexts.has(typeof gq === 'string' ? gq : gq.question));
      ...
      if (!isRegression && !dryRun) {
        const baseline = loadBaseline(null, config);
        const diversity = checkDiversity(questions.map(q => typeof q === 'string' ? q : q.question), baseline);
      ...
      if (dryRun) {
        for (const q of questions) {
          ...
          log(`    - ${qt}`);
          const score = scoreQuestion({ question: qt, toolCalls: [], errors: [], debugLogErrors: [], answer: '[dry run]' }, config);
          personaResult.questions.push({ question: qt, questionObj: typeof q === 'object' ? q : null, toolCalls: [], errors: [], debugLogErrors: [], answer: '[dry run]', score });
        }
      ...
      for (const question of questions) {
        ...
        for (const error of result.errors) {
          personaErrors.push({ persona: persona.id, question: result.question, ...error });
        }
      }
