...
async function mergeFixBranches(branches, config) {
  ...
  if (diffs.length === 1) {
    ...
    // Multiple branches — reviewer merges all diffs
    const diffSummary = diffs.map(d => `### ${d.branch}\n\`\`\`diff\n${d.diff.slice(0, 3000)}\n\`\`\``).join('\n\n');

    log(`    ${diffs.length} branches with changes — reviewer merging...`);
    await claude(
      [
        `${diffs.length} parallel fixers each made independent changes to the MCP server.`,
        `\n${diffSummary}`,
        `\nRead the current files, then Edit to apply all changes.`,
      ].join('\n'),
      {
...
export async function run(config, args = {}) {
  ...
  if (isRegression) {
    ...
    regressionPrompts = regressionBaseline.prompts;
  ...
  if (!useFixedSets && goldenSet.prompts.length > 0 && !isRegression) {
    ...
    if (lastBaseline) {
      ...
      const lastGoldenResults = lastBaseline.prompts.filter(q =>
    ...
    for (let i = 0; i < promptsToRun.length; i += concurrency) {
      ...
      const chunkResults = await Promise.all(
        chunk.map(q => {
          ...
          return runAndGrade(q.promptObj || q.prompt, persona);
        })
      );
  ...
  // === LEGACY MODE (no prompt-set.json) ===
    ...
    async function runPersona(persona) {
      ...
      for (const p of prompts) {
        const result = await runAndGrade(p, persona);
  ...
  if (useFixedSets && !runInvalid && !dryRun) {
    ...
    if (graduated.length > 0) {
      ...
      if (!noEscalate) {
        ...
        const escalationResults = await escalate(allPs, fullContext, config, runDateContext);
  ...
  if (!runInvalid && !dryRun && !noEscalate) {
    ...
    if (forceEscalate || currentAllPassed) {
      ...
      if (forceEscalate || streak.triggered) {
        ...
        const escalationResults = await escalate(streak.allPassingPrompts, fullContext, config, runDateContext);
      ...
      if (!args.noCompete && streak.streak >= competitionThreshold) {
        const passingLog = streak.allPassingPrompts
          ...
          .join('\n');
        await runCompetition({ passingPrompts: passingLog, fullContext }, config);
      }
    }
  }
