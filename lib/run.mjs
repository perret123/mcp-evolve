...
export async function run(config, args = {}) {
  ...
  if (!effectiveSkipFixer && !dryRun) {
    ...
    if (fixableErrors.length > 0) {
      ...
      // Run fixers in PARALLEL — each question's first error gets its own fixer
      const fixPromises = fixableErrors.map(fe => {
        const error = fe.errors[0]; // one fix per question
        log(`  Fixing [${fe.persona.id}] ${error.tool}...`);
        return fixError(error, fe.debugLogErrors, config)
          .then(() => log(`  Fixed [${fe.persona.id}] ${error.tool}`))
          .catch(err => log(`  Fix failed [${fe.persona.id}]: ${err.message || err}`));
      });
