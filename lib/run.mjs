...
async function generateQuestions(persona, config, fullContext, runDateContext) {
...
async function answerQuestion(question, persona, config, runDateContext) {
...
async function gradeAnswer(question, result, persona, config, runDateContext) {
...
export async function run(config, args = {}) {
  ...
  if (goldenSet.questions.length > 0 && !isRegression) {
    ...
    for (const r of allResults) {
      ...
      for (const q of r.questions) {
        const mcpErrors = (q.errors || []).filter(e =>
          e.tool !== 'cli' && !NON_FIXABLE_HARNESS_TOOLS.has(e.tool)
        );
  ...
  if (!runInvalid && !dryRun && !noEscalate) {
    ...
    if (forceEscalate || currentAllPassed) {
      ...
      if (forceEscalate || streak.triggered) {
        ...
        await escalate(streak.allPassingQuestions, fullContext, config, runDateContext);
      }
