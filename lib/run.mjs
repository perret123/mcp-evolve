...
async function gradeAnswer(question, result, config) {
...
async function mergeFixBranches(branches, config) {
  ...
  for (const b of branches) {
    try {
      ...
      execS(`git merge "${b.branchName}" --no-edit`, {
        cwd: config.projectRoot, encoding: 'utf-8', timeout: 10_000,
      });
      log(`    Merged ${b.branchName}`);
    } catch {
      // Merge conflict — use claude to resolve
      log(`    Conflict merging ${b.branchName} — resolving...`);
      try {
        const conflictFiles = execS('git diff --name-only --diff-filter=U', {
          cwd: config.projectRoot, encoding: 'utf-8', timeout: 5_000,
        }).trim();

        if (conflictFiles) {
          await claude(
            `Git merge conflict in: ${conflictFiles}\nBoth sides are MCP server fixes. Accept ALL changes from both sides — combine them. Resolve every conflict marker (<<<<<<< ======= >>>>>>>) by keeping both versions merged together.`,
            {
              systemPrompt: 'Resolve git merge conflicts by combining both sides. Use Read to see conflicts, Edit to fix them. Keep all changes from both branches.',
              allowedTools: 'Read,Edit,Grep,Bash',
              timeout: 120_000,
              cwd: config.projectRoot,
            },
          );
          execS('git add -A', { cwd: config.projectRoot, encoding: 'utf-8', timeout: 5_000 });
          execS('git commit --no-edit', { cwd: config.projectRoot, encoding: 'utf-8', timeout: 5_000 });
          log(`    Resolved conflicts in ${b.branchName}`);
        }
      } catch (resolveErr) {
        // Abort merge if resolution fails
        try { execS('git merge --abort', { cwd: config.projectRoot, encoding: 'utf-8', timeout: 5_000 }); } catch {}
        log(`    Failed to resolve ${b.branchName}: ${resolveErr.message || resolveErr}`);
      }
    }
  ...
  async function runPersona(persona) {
    ...
    for (const question of questions) {
      ...
      if (!args.skipGrading && result.errors.length === 0) {
        const gradingIssues = await gradeAnswer(question, result, config);
        if (gradingIssues) {
