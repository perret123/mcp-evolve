...
import {
  ...
  compareToBaseline, printRegressionReport, computeDistributionDrift, getPreviousPrompts,
  checkStreak,
...
export async function run(config, args = {}) {
  ...
  if (!isRegression && !dryRun) {
    ...
    log(`Running ${promptsToRun.length} prompts${concurrency < promptsToRun.length ? ` (concurrency: ${concurrency})` : ' in PARALLEL'}`);
  ...
  runState.promoter = { nominated: [], skipped: [], unmatched: [], duplicates: [], parseErrors: [], promoterOutput: null };
  ...
  if (!runInvalid && !dryRun && !isRegression) {
    ...
    const failingAll = promoterFailingStore.entries || [];
