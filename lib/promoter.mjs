 ...
 *
 * @param {object} args
 * @param {{nominations: object[], skipped: object[], parseErrors: string[]}} args.promoterOutput
 ...
 */
export function applyPromoterDecisions({ promoterOutput, candidates, config, runId }) {
  const nominated = [];
  const skipped = [...(promoterOutput.skipped || [])];
  const unmatched = [];
  const duplicates = [];
  ...
  const raw = Number(config.maxPromotionsPerRun);
  const cap = Math.max(0, Number.isFinite(raw) ? raw : 3);

  for (const n of promoterOutput.nominations || []) {
...
function buildPromoterPayload({ candidates, currentGolden, failingEntries, config }) {
  ...
  const candidateList = candidates.map(c => {
    const toolsUsedRaw = c.toolCalls || c.scorePost?.toolsUsedList || [];
    const toolsUsed = toolsUsedRaw.slice(0, 10).map(t =>
      typeof t === 'string' ? t : (t?.tool || String(t))
    );
 ...
 * @returns {Promise<{nominated: object[], skipped: object[], unmatched: object[], duplicates: object[], parseErrors: string[], promoterOutput: object|null}>}
 */
export async function runPromoter({ candidates, currentGolden, failingEntries, config, runId }) {
  if (!candidates || candidates.length === 0) {
    ...
    return { nominated: [], skipped: [], unmatched: [], duplicates: [], parseErrors: [], promoterOutput: null };
  }
  ...
  try {
    ...
    return { nominated: [], skipped: [], unmatched: [], duplicates: [], parseErrors: [`LLM call failed: ${err.message || err}`], promoterOutput: null };
  }

  if (typeof output === 'string' && output.startsWith('ERROR:')) {
    log(`  [promoter] claude CLI returned error: ${output.slice(0, 200)}`);
    return { nominated: [], skipped: [], unmatched: [], duplicates: [], parseErrors: [output.slice(0, 500)], promoterOutput: null };
  }
  ...
  const summary = applyPromoterDecisions({ promoterOutput: parsed, candidates, config, runId });
  ...
  return { ...summary, parseErrors, promoterOutput: parsed };
}
