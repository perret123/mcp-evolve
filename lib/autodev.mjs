...
import { scoreQuestion, isActionRequest, isPassingScore } from './eval.mjs';
...
/**
 * Run auto-dev for a list of blocked golden questions.
 * Each question gets its own branch.
 *
 * @returns {Array<{persona, question, branch, verdict, diff}>}
 */
export async function autoDev(blockedQuestions, config) {
  ...
  for (const bq of blockedQuestions) {
    ...
    log('');
    log('='.repeat(60));
    log(`AUTO-DEV: [${bq.persona}] ${bq.question.slice(0, 70)}...`);
    ...
    try {
      ...
      const prompt = [
        `A test question has failed ${bq.consecutiveFails || '3+'} times. The surface fixer and deep fixer both tried and failed.`,
        ...
        `4. If the tool can't do what the question asks, add the missing capability`,
        '',
        `**Rules:**`,
      ...
      if (!diff) {
        ...
        results.push({ persona: bq.persona, question: bq.question, branch: null, verdict: 'NO_CHANGES', diff: '', investigationOutput: output.slice(-500) });
        continue;
      }
      ...
      // Replay the question against the worktree's MCP server
      let replayPassed = false;
      if (buildOk) {
        log('  Replaying question against fixed code...');
        try {
          const persona = { id: bq.persona, name: bq.persona, role: bq.group || 'train' };
          const replayOutput = await claude(
            `User question: ${bq.question}`,
            {
          ...
          const score = scoreQuestion({ question: bq.question, ...result }, config);
      ...
      if (replayPassed) {
        // Regression check: run ALL non-blocked golden questions on the branch
        ...
        try {
          ...
          const otherGolden = (gs.questions || []).filter(q => !q.blocked && q.question !== bq.question);
          ...
          if (otherGolden.length > 0) {
            ...
            const regResults = await Promise.all(otherGolden.map(async (gq) => {
              ...
              const s = scoreQuestion({ question: gq.question, ...r }, config);
              return { persona: gq.persona, question: gq.question, passed: isPassingScore(s) };
            }));

            for (const rr of regResults) {
              if (rr.passed) regPass++;
              else { regFail++; log(`    REGRESSION: [${rr.persona}] ${rr.question.slice(0, 60)}`); }
            }

            log(`  Regression: ${regPass} passed, ${regFail} failed out of ${otherGolden.length} golden questions`);
            if (regFail > 0) {
              regressionPassed = false;
              log('  Fix causes regressions — aborting');
            }
          } else {
            log('  No other golden questions to check');
          }
        } catch (err) {
        ...
        if (!regressionPassed) {
          cleanup(worktreePath, branchName, config);
          results.push({ persona: bq.persona, question: bq.question, branch: null, verdict: 'FIX_CAUSES_REGRESSION', diff });
          continue;
        }
        ...
        results.push({ persona: bq.persona, question: bq.question, branch: branchName, verdict: 'FIX_READY', diff: fullDiff });
      } else {
        log('  Fix did not resolve the issue — cleaning up');
        cleanup(worktreePath, branchName, config);
        results.push({ persona: bq.persona, question: bq.question, branch: null, verdict: 'FIX_FAILED', diff });
      }

    } catch (err) {
      log(`  Auto-dev error: ${err.message?.slice(0, 200)}`);
      cleanup(worktreePath, branchName, config);
      results.push({ persona: bq.persona, question: bq.question, branch: null, verdict: 'ERROR', diff: '' });
    }
  }
