...
export function compareToBaseline(currentResults, baseline) {
  ...
  for (const bq of baseline.prompts) {
...
export function checkDiversity(newPrompts, baseline) {
  ...
  const basePrompts = baseline.prompts.map(q => q.prompt);
  let totalSim = 0;
  let comparisons = 0;
  const duplicates = [];
...
export function addTrainPrompts(ps, newPrompts, config) {
  for (const q of newPrompts) {
    ...
    ps.prompts.push({
      persona: q.persona,
      prompt: q.prompt,
      promptObj: { prompt: q.prompt, probe: q.probe, invariant: q.invariant, probeType: q.probeType },
      group: 'train',
      consecutivePasses: 0,
      graduatedAt: null,
    });
