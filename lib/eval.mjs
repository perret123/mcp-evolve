  ...
  for (const r of results) {
    for (const q of r.questions) {
      questions.push({
        ...
        question: q.question,
        questionObj: q.questionObj || null,
        score: q.score,
      });
    }
  }
...
function tokenFrequencyDistribution(questions) {
  const freq = {};
  let total = 0;
...
export function computeDistributionDrift(newQuestions, referenceQuestions, threshold = 0.4) {
  if (referenceQuestions.length === 0) return { drift: 0, noveltyRatio: 0, highDrift: false, topNovelTokens: [] };

  const ref = tokenFrequencyDistribution(referenceQuestions);
  const novel = tokenFrequencyDistribution(newQuestions);
...
export function promoteToGoldenSet(persona, question, fixCycle, config, questionObj = null) {
...
export function createQuestionSet(questions, goldenPercent = 30) {
  ...
  return {
    ...
    questions: questions.map((q, i) => ({
      ...
      questionObj: typeof q.question === 'object' ? q.question : (q.questionObj || null),
...
export function updateQuestionSetAfterRun(qs, results, config) {
  ...
  for (const q of qs.questions) {
    ...
    for (const r of results) {
      ...
      const found = r.questions.find(rq =>
...
export function addTrainQuestions(qs, newQuestions, config) {
  for (const q of newQuestions) {
    ...
    qs.questions.push({
      persona: q.persona,
      question: q.question,
      questionObj: { question: q.question, probe: q.probe, invariant: q.invariant, probeType: q.probeType },
      group: 'train',
      consecutivePasses: 0,
      graduatedAt: null,
    });
