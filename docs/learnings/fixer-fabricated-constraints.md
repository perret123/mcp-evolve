...
## Second addendum (Run 17, Round 9)

After the manual revert between Run 16 and 17, I watched to see if the strengthened `prompts/fixer.md` rule ("NEVER fabricate domain constraints") would hold. **It did not.** During Run 16's model-error fixer phase (which was still running when the revert happened), sonnet re-added the guard — this time **inlined directly** in the manage_guest handler (without the helper function) in two places. The new code had a comment:

```typescript
// Occupancy check for walk-ins: seating a new party at an already-occupied table is likely wrong.
```

**"likely wrong"** — sonnet was explicitly uncertain, and the prompt rule said "when in doubt, improve the description instead". Sonnet chose to add the block anyway.

### Why the prompt rule failed

The rule was too abstract. Sonnet saw 23 error messages in its input, most containing `⛔ Table X is already occupied`, and concluded: "There's a clear pattern of failures that a validation check would prevent." It never checked **where those error messages came from**. The errors were generated by the very code it was editing — they were a reflection of the fabricated guard, not real backend failures.

This is a **circular-error trap**: fixer sees errors → fixer "fixes" them by implementing the validation that caused them → next run produces the same errors → fixer reinforces.

### The stronger rule

The new version of `prompts/fixer.md` makes this a **mandatory self-check**: before writing any rejection, the fixer MUST:

1. Grep the source for the error text it's about to write — if it exists, the guard is already there
2. Locate the backend handler and verify the backend actually rejects the condition
3. Check `git log --oneline -10 <file>` for recent commits that REMOVED the same guard
4. Re-read its input errors with the hypothesis that they might come from MCP-side code, not the backend

Abstract "don't fabricate" rules don't work for a pattern-matching LLM. Concrete checklists with specific grep/git commands do.

### The structural problem

Even with a perfect prompt, the fixer's incentive structure still favors adding guards: every guard that stops an LLM retry loop makes tests score higher. The prompt can raise the bar, but the fundamental tension remains. Longer term, this needs a **review step** that specifically audits new rejection logic against the backend — a second agent whose job is to find fabricated constraints, not to make tests green.
