...
- `lib/` — Core library
  ...
  - `Codex.mjs` — Codex CLI wrapper, stream-json parser (with tool result capture)
...
SEED → GENERATE (Sonnet) → ANSWER (Opus, name-only user context)
  ...
  → FIX (parallel, git worktrees) → MERGE (Codex) → REPLAY → PROMOTE to golden set
...
- **Parallel fixers** in isolated git worktrees, merged by Codex
